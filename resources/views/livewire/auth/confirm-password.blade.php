<x-layouts::auth :title="__('authui.confirm_title')">
    <div>
        <h2 class="text-center text-slate-900 mb-2 font-extrabold tracking-tight text-2xl">{{ __('authui.confirm_header') }}</h2>
        <p class="text-center text-slate-500 text-sm font-semibold mb-6">{{ __('authui.confirm_desc') }}</p>
        ...
        <form method="POST" action="{{ route('password.confirm.store') }}">
            ...
            <button type="submit" class="w-full bg-blue-600 text-white py-4 rounded-2xl font-extrabold text-xs tracking-widest uppercase hover:bg-slate-900 transition" data-test="confirm-password-button">
                {{ __('authui.confirm_button') }}
            </button>
        </form>
    </div>
</x-layouts::auth>
